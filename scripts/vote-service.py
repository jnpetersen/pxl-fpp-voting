...
def get_all_settings():
    url = 'http://127.0.0.1/api/configfile/plugin.pxl-voting'
    response = requests.request("GET", url, headers={}, data={}, timeout=(10, 10))
...
def get_cron_comment():
    return 'pxl-fpp-voting-cron-job'

def get_cron_command():
    return '/usr/bin/python3 /home/fpp/media/plugins/pxl-fpp-voting/scripts/vote-service.py {} > /dev/null &'.format(private_key)
...
def sqlite3_connection():
    try:
        conn = sqlite3.connect('/home/fpp/media/plugins/pxl-fpp-voting/pxl-vote.db')
        with closing(conn.cursor()) as cursor:
